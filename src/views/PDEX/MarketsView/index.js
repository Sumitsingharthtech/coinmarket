...
import {
  getCandles, getLiquidity, getVolumes, getVolumesWithoutCrossPool, getCrossPoolCandles, getCrossPoolVolumes
} from '../../../actions/candlesActions';
import Trades from './Trades';
import { getTradesByPair } from '../../../actions/tradesActions';
...
function MarketsView({ location, match: { params } }) {
  const classes = useStyles();
  const pair = params.pair ? params.pair : DEFAULTS.PAIR;
  ...
  const { candles, liquidity, volumes, volumesWithoutCrossPool } = useSelector((state) => state.candle);
  const { tradesByPair, tradesByPairCount } = useSelector((state) => state.trade);
  let currentCandles = candles.byId[`${pair}-${interval.granularity}`];
  const currentVolumes = volumes.byId[`${pair}-${interval.granularity}`];
  const currentVolumesWithoutCrossPool = isCrossPool ? null : volumesWithoutCrossPool.byId[`${pair}-${interval.granularity}`];
  ...
  useEffect(() => {
    dispatch(getTradesByPair(pair, limit, page * limit, hideSmallTrades));
    const timer = setInterval(
      () => dispatch(getTradesByPair(pair, limit, page * limit, hideSmallTrades)),
      30000
    );
  ...
  useEffect(() => {
    if (!isCrossPool) {
      dispatch(getCandles(pair, interval.granularity, from(), to(), reversedChart));
      dispatch(getVolumes(pair, interval.granularity, from(), to(), reversedChart));
      dispatch(getVolumesWithoutCrossPool(pair, interval.granularity, from(), to(), reversedChart));
      const timer = setInterval(
        () => {
          dispatch(getCandles(pair, interval.granularity, from(), to(), reversedChart));
          dispatch(getVolumes(pair, interval.granularity, from(), to(), reversedChart));
    ...
    dispatch(getCrossPoolCandles(pair, interval.granularity, from(), to(), reversedChart));
    dispatch(getCrossPoolVolumes(pair, interval.granularity, from(), to(), reversedChart));
    const timer = setInterval(
      () => {
        dispatch(getCrossPoolCandles(pair, interval.granularity, from(), to(), reversedChart));
        dispatch(getCrossPoolVolumes(pair, interval.granularity, from(), to(), reversedChart));
  ...
  useEffect(() => {
    if (!isCrossPool) {
      dispatch(getLiquidity(pair, base(pair), interval.granularity, from(), to()));
      dispatch(getLiquidity(pair, quote(pair), interval.granularity, from(), to()));
      const timer = setInterval(
        () => {
          dispatch(getLiquidity(pair, base(pair), interval.granularity, from(), to()));
          dispatch(getLiquidity(pair, quote(pair), interval.granularity, from(), to()));
  ...
  const onIntervalSelected = (newInterval) => {
    if (newInterval && newInterval.value !== interval.value) {
      ...
      history.push(`/pdex/markets/${pair}/${newInterval.value}${reversedChart ? '?reversed=true' : ''}`);
    }
  };
  ...
  const onPairSelected = (newPair) => {
    if (newPair && newPair !== pair) {
      ...
      setPage(0);
      setLimit(10);
      history.push(`/pdex/markets/${newPair}/${interval.value}${reversedChart ? '?reversed=true' : ''}`);
    }
  };
  ...
  const onChartReversed = () => {
    ...
    history.push(`/pdex/markets/${pair}/${interval.value}${!reversedChart ? '?reversed=true' : ''}`);
  };
  ...
  return (
    ...
    >
      <Container maxWidth={false} className={classes.container}>
        <Header isCrossPool={isCrossPool} pair={pair} pairs={pairs} onPairSelected={onPairSelected} intervals={intervals} interval={interval} onIntervalSelected={onIntervalSelected} />
        <Box mt={1}>
          <Grid
            container
            spacing={3}
          >
            ...
            >
              ...
              <Divider />
              <Box mt={2} mb={0}>
                {currentTab === 'chart' && <CandleChart isCrossPool={isCrossPool} pair={reversedChart ? inversePair(pair) : pair} interval={interval} candles={currentCandles} volumes={currentVolumes} volumesWithoutCrossPool={currentVolumesWithoutCrossPool} onChartReversed={onChartReversed} />}
